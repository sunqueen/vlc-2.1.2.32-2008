...
int (poll) (struct pollfd *fds, unsigned nfds, int timeout)
{
    ...
    fd_set *rdset = (fd_set *)malloc (setsize);			// sunqueen modify
    fd_set *wrset = (fd_set *)malloc (setsize);			// sunqueen modify
    fd_set *exset = (fd_set *)malloc (setsize);			// sunqueen modify
...
# define FD_SETSIZE (nfds)
...
#endif
        if (fds[i].events & __POLLIN)			// sunqueen modify
            FD_SET (fd, rdset);
        if (fds[i].events & __POLLOUT)			// sunqueen modify
            FD_SET (fd, wrset);
        if (fds[i].events & __POLLPRI)			// sunqueen modify
            FD_SET (fd, exset);
    }
    ...
    {
        int fd = fds[i].fd;
        fds[i].revents = (FD_ISSET (fd, rdset) ? __POLLIN : 0)
                       | (FD_ISSET (fd, wrset) ? __POLLOUT : 0)
                       | (FD_ISSET (fd, exset) ? __POLLPRI : 0);			// sunqueen modify
    }
#ifdef _WIN32
    free (exset);
    free (wrset);
